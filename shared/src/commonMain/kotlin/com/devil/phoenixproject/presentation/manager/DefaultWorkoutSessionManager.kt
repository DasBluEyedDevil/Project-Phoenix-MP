...
class DefaultWorkoutSessionManager(
    ...
    init {
        ...
        // Collectors #1-2 (routines loader, exercise importer) moved to RoutineFlowManager.
        // RoutineFlowManager is constructed first (above), so its init block runs before this one,
        // preserving the original collector ordering.
    ...
    // ===== Delegations to RoutineFlowManager =====
    ...
    fun clearCycleContext() = routineFlowManager.clearCycleContext()
    ...
    private fun startRestTimer() {
        ...
        coordinator.restTimerJob = scope.launch {
            ...
            val nextStep = if (routine != null) {
                routineFlowManager.getNextStep(routine, coordinator._currentExerciseIndex.value, coordinator._currentSetIndex.value)
            } else null
            ...
            val isInSupersetTransition = routineFlowManager.isInSuperset() && !routineFlowManager.isAtEndOfSupersetCycle()
            val isStillInSupersetWorkout = routineFlowManager.isInSuperset() && nextExerciseFromStep != null &&
            ...
            val restDuration = if (isInSupersetTransition || isStillInSupersetWorkout) {
                routineFlowManager.getSupersetRestSeconds().coerceAtLeast(5) // Min 5s for superset transitions
            } else {
            ...
            val isLastExerciseOverall = routineFlowManager.calculateIsLastExercise(isSingleExercise, currentExercise, routine)
            ...
            while (currentTimeMillis() < endTimeMs && isActive) {
                ...
                val nextName = routineFlowManager.calculateNextExerciseName(isSingleExercise, currentExercise, routine)
            ...
            if (autoplay) {
                ...
                coordinator._workoutState.value = WorkoutState.Resting(
                    ...
                    nextExerciseName = routineFlowManager.calculateNextExerciseName(isSingleExercise, currentExercise, routine),
                    isLastExercise = isLastExerciseOverall,
                    currentSet = displaySetIndex,
                    totalSets = displayTotalSets,
                    isSupersetTransition = isInSupersetTransition || isStillInSupersetWorkout,
